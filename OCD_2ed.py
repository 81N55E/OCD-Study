...
practice = data.TrialHandler(nReps=1, method='random', 
    ...
    trialList=data.importConditions(u'trialListPrac.xlsx'),
...
trials = data.TrialHandler(nReps=1, method='random', 
    extraInfo=expInfo, originPath=-1,
    trialList=data.importConditions(u'trialList.xlsx'),
...
for thisTrial in trials:
    ...
    while continueRoutine and routineTimer.getTime() > 0:
        ...
        if t >= 0.0 and text_3.status == NOT_STARTED:
            ...
            text_3.frameNStart = frameN  # exact frame index
            text_3.setAutoDraw(True)
        frameRemains = 0.0 + .998- win.monitorFramePeriod * 0.75  # most of one frame period left
        if text_3.status == STARTED and t >= frameRemains:
            text_3.setAutoDraw(False)
    ...
    # keep track of which components have finished
    TrialComponents = [text_4, key_resp_2, text_11]
